...
2. Device connect picks a folder, then inspects for `dev/design-playground`. If none exists, ask **No Playground Worktree for this repo** with red **Upload or Add Template**, buttons **Yes** / **Change Repo**. Yes creates the worktree from staging, then polls for a design bible (`docs/DESIGN.md`, not `theme.css`). Change Repo clears the repo field and returns to New project. After the worktree exists, intake reads `docs/DESIGN-BIBLE.md`. If present, the bible is found (field shows the DESIGN.md path and status) and Save writes nothing. If absent but design files exist, intake immediately asks **Integrate existing** or **Start fresh**. Otherwise Design Bible is **Upload** or **+ Template**. Template writes the stripped skeleton in `docs/bible-language.md` (no Vanyshr tokens). If the repo already has design files, Template asks **Integrate existing** or **Start fresh**. Integrate keeps the playground template as the format: a DESIGN.md already in template shape is left alone; an old-shaped DESIGN.md is parsed and used to fill the template (color names stay Core Colors). Start fresh writes the empty skeleton. Until Save, the field reads **Un-Committed**. Save writes the bible if needed, writes or appends `DESIGN-BIBLE.md`, commits in the worktree, creates the playground project, and opens Project Settings. On the Design Bible tab, Integrate appends a log entry (token value changes, files written); **Mark solidified** (enabled once DESIGN.md, tokens.css, and COMPONENTS.md are Ready) and **Reopen as draft** flip the header status and log it.
